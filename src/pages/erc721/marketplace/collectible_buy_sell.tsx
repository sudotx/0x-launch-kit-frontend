...
interface CollectibleBuySellProps {
    collectibleId: string;
}

const CollectibleBuySell: React.FC<CollectibleBuySellProps> = ({ collectibleId, ...restProps }) => {
    const dispatch: AppDispatch = useDispatch();
    const { address } = useAccount()
    const ethAccount = address;
    const collectible = mockCollectible;
    ...
    const onCancel = useCallback(async () => {
        ...
        try {
            if (collectible?.order) {
                await dispatch(cancelOrderCollectible(collectible.order));
            }
        } catch (err) {
    ...
    const onBuy = useCallback(async () => {
        ...
        try {
            if (collectible) {
                dispatch(startBuyCollectibleSteps({ collectible }));
            }
        } catch (err) {
            window.alert(`Could not buy the specified order`);
        } finally {
            setIsLoading(false);
        }
    }, [collectible, dispatch]);
