...
import { ButtonVariant, Collectible, OrderSide } from '../../../util/types';
...
const iconETH = () => (
    <svg width="12" height="17" viewBox="0 0 12 17" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M6.08877 12.4737L6.00244 12.5163V16.4753L6.08877 16.6456L11.2683 9.45117L6.08877 12.4737Z" fill="#38393A" />
...
const CollectibleSellModalContainer: React.FC<OwnProps> = ({ theme }) => {
    const [startPrice, setStartPrice] = useState<BigNumber | null>(null);
    const [endingPrice, setEndingPrice] = useState<BigNumber | null>(null);
    const [expirationDate, setExpirationDate] = useState<BigNumber>(tomorrow());
    const [shouldIncludeEndPrice, setShouldIncludeEndPrice] = useState(false);

    const dispatch = useDispatch<AppDispatch>();
    const currentCollectible = useSelector(getSelectedCollectible);

    const closeModal = () => {
        setStartPrice(null);
        setEndingPrice(null);
        setExpirationDate(tomorrow());
        setShouldIncludeEndPrice(false);
        dispatch(selectCollectible(null));
    };
    ...
    const openStepsModals = async () => {
        if (!currentCollectible || !startPrice) {
            return;
        }
        closeModal();
        await dispatch(
            startSellCollectibleSteps({
    ...
    return (
        <Modal isOpen={currentCollectible !== null} style={theme.modalTheme as any} onRequestClose={closeModal}>
            <ModalTitleWrapper>
            ...
            <ModalContent>
                <CollectibleMainInfoWrapper>
                    ...
                    <CollectibleMainInfo>
                        <div>
                            <CollectibleMainInfoTitle>{currentCollectible ? currentCollectible.name : ''}</CollectibleMainInfoTitle>
                            <CollectibleLink href={currentCollectible ? currentCollectible.assetUrl : ''} target="_blank">
                ...
                <FormRow>
                    ...
                    <SwitchWrapper isActive={shouldIncludeEndPrice}>
                        <SwitchInput
                            checked={shouldIncludeEndPrice}
                            onChange={e => setShouldIncludeEndPrice(e.target.checked)}
                            type="checkbox"
                        />
                        <Switch />
                    </SwitchWrapper>
                ...
                <FormRow>
                    ...
                    <SelectStyled onChange={e => setExpirationDate(new BigNumber(e.target.value))}>
                ...
                <ButtonStyled onClick={openStepsModals} disabled={isFormInvalid()} variant={ButtonVariant.Error}>
                    Sell
                </ButtonStyled>
            </ModalContent>
        </Modal>
...
const CollectibleSellModal = withTheme(CollectibleSellModalContainer);
